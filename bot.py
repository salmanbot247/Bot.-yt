...
class ConsoleSpy:
    ...
    def write(self, text):
        ...
        if "google.com/device" in text and "code:" in text.lower():
            bot.send_message(
                CHAT_ID, 
                f"🚨 **YOUTUBE LOGIN REQUIRED** 🚨\n\n```text\n{text.strip()}\n```\n\n👆 **Link open karein aur code enter karein. Bot aapka wait kar raha hai!**",
                parse_mode="Markdown"
            )
    def flush(self):
...
# 🛡️ THE IRON SHIELD: Admin Check Middleware
@bot.message_handler(func=lambda message: str(message.chat.id) != str(CHAT_ID))
def block_strangers(message):
    bot.reply_to(message, "🚫 **Access Denied!** You are not the Admin of this God Mode Bot.")
...
@bot.message_handler(func=lambda m: str(m.chat.id) == str(CHAT_ID))
